...
def preprocessing_and_training(EPOCHS,INIT_LR,BS):
	...
	train_x = train_x.reshape(len(df), 64,64,1).astype('float32')
	train_y = to_categorical(df.label.values,num_classes=3)
	...
	model = CNNmodel.build(width=64, height=64, depth=1, classes=3)
